                ...
                <div class="col-lg-12">
                    ...
                    <c:if test="${!empty staffTypes}">
                        <table class="table table-striped table-list-search">
                            ...
                            <tbody>
                            <c:forEach items="${staffTypes}" var="type">
                                <tr>
                                    <td>${type.getTypeName()}</td>
                                    <td>${type.getManager().getTypeName()}</td>
                                    <td>${type.getUser().getLogin()}</td>
                                    <td>
                                        <div class='pull-right'>
                                            <a class="btn btn-success" href="<c:url value='/staff/type/${type.getId()}/edit' />" >Edit</a>
                                            <a class="btn btn-danger" href="<c:url value='/staff/type/${type.getId()}/delete' />" >Delete</a>
                                        </div>
                                    </td>
                                </tr>
                            </c:forEach>
                    ...
                    <button type="button" class="btn btn-primary" onclick="window.location.href='/staff/type/add'">Add Staff Type</button>
                </div>
            </div>
        </div>
    </div>
